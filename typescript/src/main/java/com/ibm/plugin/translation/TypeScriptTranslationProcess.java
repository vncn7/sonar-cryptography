...
import com.ibm.plugin.translation.translator.TypeScriptTranslator;
import java.util.Collections;
import java.util.List;
import javax.annotation.Nonnull;

/** Three-stage translation pipeline for TypeScript: translate → reorganize → enrich. */
public final class TypeScriptTranslationProcess
        extends ITranslationProcess<
                TypeScriptCheck, TypeScriptTree, TypeScriptSymbol, TypeScriptScanContext> {

    public TypeScriptTranslationProcess(@Nonnull List<IReorganizerRule> reorganizerRules) {
    ...
    public List<INode> initiate(
            @Nonnull
                    DetectionStore<
                            ...
                            rootDetectionStore) {
        // 1. Translate
        final TypeScriptTranslator translator = new TypeScriptTranslator();
