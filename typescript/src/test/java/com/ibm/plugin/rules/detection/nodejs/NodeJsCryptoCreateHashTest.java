...
class NodeJsCryptoCreateHashTest extends TestBase {

    @Test
    void test() throws Exception {
        TypeScriptVerifier.verify("rules/detection/nodejs/NodeJsCryptoCreateHashTestFile.ts", this);
    }
    ...
    public void asserts(
        ...
        switch (findingId) {
            case 0 -> {
                ...
                assertThat(node.getChildren().get(DigestSize.class)).isNotNull();
                assertThat(node.getChildren().get(DigestSize.class).asString()).isEqualTo("256");
            }
            case 1 -> {
                assertThat(value0.asString()).isEqualTo("sha512");
                assertThat(node.asString()).isEqualTo("SHA512");
                assertThat(node.getChildren().get(DigestSize.class)).isNotNull();
                assertThat(node.getChildren().get(DigestSize.class).asString()).isEqualTo("512");
            }
            case 2 -> {
                assertThat(value0.asString()).isEqualTo("md5");
                assertThat(node.asString()).isEqualTo("MD5");
                assertThat(node.getChildren().get(DigestSize.class)).isNotNull();
                assertThat(node.getChildren().get(DigestSize.class).asString()).isEqualTo("128");
            }
